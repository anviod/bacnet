...
func TestUdpDataLink(t *testing.T) {
	c, err := NewClient(&ClientBuilder{Interface: interfaceName})
	if err != nil {
		t.Skipf("Skipping TestUdpDataLink: %v", err)
	}
	c.Close()
...
func TestMac(t *testing.T) {
	...
	log.Logger.Info("mac decoded", zap.Uint16("port", p))
}

func TestServices(t *testing.T) {
	c, err := NewClient(&ClientBuilder{Interface: interfaceName})
	if err != nil {
		t.Skipf("Skipping TestServices: %v", err)
	}
	defer c.Close()
...
func testWritePropertyService(c Client, t *testing.T) {
	...
	s, ok := d.(string)
	if !ok {
		log.Logger.Fatal("unexpected return type", zap.String("type", fmt.Sprintf("%T", d)))
	}

	if s != targetName {
		log.Logger.Fatal("write to name failed", zap.String("target", targetName), zap.String("actual", s))
	}
...
func TestDeviceClient(t *testing.T) {
	c, err := NewClient(&ClientBuilder{Interface: interfaceName})
	if err != nil {
		t.Skipf("Skipping TestDeviceClient: %v", err)
	}
	go c.ClientRun()
	wh := &WhoIsOpts{
		GlobalBroadcast: false,
